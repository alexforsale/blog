...
** Theme
   #+begin_src emacs-lisp
     ...
     (cond ((and (featurep 'config-doom-themes)
           ...
           ((custom-theme-name-valid-p +config-standard-theme)
            (+config/load-theme +config-standard-theme t)))
   #+end_src
** Footer
   #+begin_src emacs-lisp
     (provide 'config-user)
...
**** treemacs
     ...
     Keybinding
     #+begin_src emacs-lisp
       (global-set-key (kbd "C-x M-j") 'treemacs)
       (global-set-key (kbd "C-x M-SPC") '+config/treemacs-back-and-forth)
     #+end_src
     Misc
     #+begin_src emacs-lisp
       ;; projectile
...
*** custom hook for org-mode src-code-block
    #+begin_src emacs-lisp
      (defun +config/electric-indent-org-hook ()
        (add-hook '(electic-indent-mode 0) 'org-in-src-block-p nil :local))
...
**** notmuch
     ...
     #+begin_src emacs-lisp
       ...
       (defun +my/mbsync-poll (&optional args)
         "Poll offlineimap with ARGS if provided."
         (setq-local output-buffer "*mbsync*"
                     error-buffer "*mbsync-error*"
                     command "mbsync")
         (if args
             (progn
               (async-shell-command (mapconcat #'identity `(,command ,@args) " ") output-buffer error-buffer)
               (async-shell-command "notmuch new --verbose" output-buffer error-buffer))
           (progn
             (async-shell-command (mapconcat #'identity `(,command ,@args) " ") output-buffer error-buffer)
             (async-shell-command "notmuch new --verbose" output-buffer error-buffer))))
     #+end_src
     ...
     #+begin_src emacs-lisp
       ...
       (defun +my/interactive-mbsync-poll (&optional args)
         "Runs mbsync interactively.
       Should be run with transient."
         (interactive
          (list (transient-args '+my/mbsync-transient)))
         (let ((command "mbsync")
               (output-buffer "*mbsync*")
               (error-buffer "*mbsync-error*"))
           (if args
               (progn
                 (async-shell-command (mapconcat #'identity `(,command ,@args) " ") output-buffer error-buffer)
     ...
     #+begin_src emacs-lisp
       ...
       (transient-define-prefix +my/mbsync-transient ()
         "Mbsync transient account chooser"
         :man-page "mbsync"
         :incompatible '(("gmail" "yahoo" "ymail" "hotmail"))
         ["Account"
          ("g" "Gmail" "gmail")
          ("y" "Yahoo" "yahoo")
          ("Y" "Ymail" "ymail")
         ...
         ["Options"
          ...
          ("-q" "quiet" "--quiet")]
         ["Actions"
          ("p" "Poll mbsync" +my/interactive-mbsync-poll)])
       ...
       (when (executable-find "mbsync")
         (define-key notmuch-common-keymap [remap notmuch-poll-and-refresh-this-buffer] #'+my/mbsync-transient))
     #+end_src
***** Footer
      #+begin_src emacs-lisp
...
** website
   :PROPERTIES:
   :header-args: :tangle (expand-file-name "site-lisp/websites.el" user-emacs-directory) :mkdirp t
   :END:
...
** blog
   :PROPERTIES:
   :header-args: :tangle (expand-file-name "site-lisp/blog.el" user-emacs-directory)
   :END:
...
** Roam Publish
   :PROPERTIES:
   :header-args: :tangle (expand-file-name "site-lisp/roam-publish.el" user-emacs-directory)
   :END:
*** commented header
    #+begin_src emacs-lisp
