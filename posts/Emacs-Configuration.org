...
***** variables
      #+begin_src emacs-lisp
        (defconst +general-leader "C-c SPC")
      #+end_src
       ...
       #+begin_src emacs-lisp
         (when (featurep 'general)
           ...
           (if (featurep 'evil)
             ...
             (progn
               (global-set-key (kbd "C-c SPC g C-p") '+git-gutter:previous-hunk)
               (global-set-key (kbd "C-c SPC g C-n") '+git-gutter:next-hunk))))
       #+end_src
***** Footer
      #+begin_src emacs-lisp
