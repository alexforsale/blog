   ...
   - slime
     #+begin_src emacs-lisp
       (when (executable-find "sbcl")
         ...
         (require 'slime)
         (when (file-exist-p (expand-file-name "slime-helper.el" +quicklisp-path)
      ...
      (setq org-publish-project-alist
            (append org-publish-project-alist
                    `(("roam"
                       ...
                       :publishing-directory ,(expand-file-name "notes" +blog-publish-directory)
                       :exclude ,(regexp-opt '("README.org" "draft" "404.org"))
                       :auto-sitemap t
                       :sitemap-filename "index.org"
    ...
    #+begin_src emacs-lisp
      ...
      (add-hook 'org-roam-graph-generation-hook
                (lambda (dot svg) (if (< (- (cadr (current-time)) +roam-publish-time) 5)
                                      (progn (copy-file svg (expand-file-name "notes/sitemap.svg" +blog-publish-directory) 't)
